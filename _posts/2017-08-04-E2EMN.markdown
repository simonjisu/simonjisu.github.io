...
### Input
1. $T_c$ 개의 단어가 포함된 한 **문장 sentence i** 는 $x_i = [x_{i1}, x_{i2}, \cdots, x_{iT_c}]$ 로 표현 할 수 있으며, 하나의 단어 는 BoW(Bag-of-Words)방식으로 인코딩 하여 vector로 바꿔준다. (**단어 사전의 index**), 이렇게 구성된 여러 문장들의 집합을 Context $\{x_i\}$라고 한다.
...
### Input Memory
1. 하나의 문장 $x_i$ 각각의 단어에 Embedding matrix $A$를 곱하여 각각의 단어를 Embedding Vectors로 변환하고 이를 모두 더하여 메모리 벡터(Memory Vector) $m_i$를 구한다. 이렇게 총 $n$개의 메모리가 만들어 진다.
