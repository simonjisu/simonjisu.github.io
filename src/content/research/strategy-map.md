---
title: "StrategyMap"
subtitle: "A process-knowledge guidance layer for dependency-sensitive intelligent agents."
subtitleKo: "절차와 순서가 중요한 일을 에이전트가 길 잃지 않고 처리하도록 돕는 작업 지도입니다."
year: "2026"
status: "Under review"
image: "/research/img/strategy-map-overview.png"
showHeroImage: false
summary: "StrategyMap turns procedural domain knowledge into a working map that helps agents navigate tasks with dependencies, state transitions, and recurring decision points."
summaryKo: "복잡한 업무 지식을 긴 프롬프트에 넣어 두는 대신, 실행 중에 펼쳐 볼 수 있는 지도로 바꿨습니다. 에이전트는 이 지도를 보며 지금 해야 할 다음 단계를 고릅니다."
...
sections:
  - key: "background"
    ...
    bodyKo:
      ...
      - >
        StrategyMap은 사람이 검토한 업무 절차를 milestone과 dependency로 표현하고, 실행 중에는 Working Map으로 completed, admissible, blocked 상태를 드러낸다. 에이전트는 전체 절차를 외워서 움직이는 것이 아니라, 현재 확인된 process state에서 다음으로 유효한 지점을 고른다. 그래서 StrategyMap은 workflow를 프롬프트에 붙이는 방식이 아니라, 업무 지식을 실행 시점의 상태 표현으로 바꾸는 방법이다.
    bodyEn:
      - >
      ...
      - >
        StrategyMap represents reviewed process knowledge as milestones and dependencies, then instantiates it as a Working Map during execution. The Working Map exposes completed, admissible, and blocked states, so the agent can choose the next valid objective from checked process progress. In this sense, StrategyMap is not just workflow text in a prompt; it turns workflow knowledge into runtime state.
  - key: "method"
    navKo: "방법"
    navEn: "Method"
    titleKo: "실행 중에 바뀌는 작업 지도"
    titleEn: "Runtime Working Map"
    bodyKo:
      - >
      ...
      - >
        실행이 시작되면 published map으로부터 session-local Working Map을 만든다. 이 Working Map은 전체 map의 dependency를 유지하면서, 현재 세션에서 어떤 milestone이 완료되었고 어떤 milestone이 다음으로 실행 가능한지 계산한다. 즉 에이전트는 전체 절차 구조를 보되, dependency와 현재 process state 때문에 admissible frontier 안에서 다음 목표를 고르게 된다.
      - >
        업데이트는 실행 가능한 상태만 대상으로 한다. 도구 호출이나 milestone transition 뒤에 관찰값이 해당 milestone의 output contract와 맞으면, 그 milestone을 satisfied로 표시하고 contract-checked output을 node memory에 기록한다. 조건이 맞지 않으면 상태를 커밋하지 않는다. 그래서 StrategyMap은 단순한 고정 그래프가 아니라, 실행 중에 확인된 증거만 반영하는 process-state interface다.
      - >
        TRA 설정에서는 Reasoning Engine과 Librarian을 분리했다. Reasoning Engine은 Working Map이 제시한 다음 milestone과 process progress에 집중하고, Librarian은 MCP 서버에서 후보 도구를 찾아 제공한다[5]. 이 분리는 StrategyMap이 정답을 직접 고르는 장치가 아니라, 에이전트가 process-valid objective를 유지한 채 필요한 도구를 찾도록 돕는 실행 프로토콜임을 보여준다.
    bodyEn:
      - >
      ...
      - >
        At the start of execution, the published map initializes a session-local Working Map. The Working Map preserves the full dependency structure while computing which milestones are completed and which milestones are currently admissible. The agent can see the process structure, but dependency state determines the admissible frontier from which the next objective should be chosen.
      - >
        Updates are committed only for admissible process progress. After a tool call or milestone transition, the system checks whether the observation satisfies the milestone output contract. If it does, the milestone is marked as satisfied and the contract-checked output is stored in node-local memory. If it does not, the Working Map state is not committed. This makes StrategyMap a runtime process-state interface, not a static graph prompt.
      - >
        In the TRA setting, the Reasoning Engine is separated from the Librarian. The Reasoning Engine focuses on the next milestone and process progress exposed by the Working Map, while the Librarian retrieves candidate tools from MCP servers [5]. This separation frames StrategyMap as an execution protocol for preserving process-valid objectives while tool provisioning is handled separately.
    figure:
      src: "/research/img/strategy-map-overview.png"
      alt: "StrategyMap runtime architecture"
      captionKo: "StrategyMap의 실행 중 작업 지도 구조."
      captionEn: "Runtime Working Map architecture in StrategyMap."
  - key: "results"
    navKo: "결과"
    navEn: "Results"
    ...
    bodyKo:
      ...
      - >
        S&OP에서는 StrategyMap이 정확도를 22.9~38.6%p 높였다. 고정 체크리스트나 고정 Strategy Map context보다, 실행 중 Working Map이 completed, admissible, blocked 상태를 유지하는 조건이 더 좋은 결과를 냈다.
      - >
    ...
    bodyEn:
      ...
      - >
        In S&OP, StrategyMap improved accuracy by 22.9 to 38.6 percentage points. Ordered checklists and static Strategy Map context underperformed the full Working Map, showing that maintaining completed, admissible, and blocked states during execution matters.
      - >
    ...
    highlightsKo:
      ...
      - "고정 체크리스트와 고정 지도보다 실행 중 작업 지도가 더 좋은 성능"
    ...
    highlightsEn:
      ...
      - "Full Working Map outperformed static checklists and static maps"
    ...
    table:
      captionKo: "S&OP ablation 결과. 같은 executor 안에서 Full SM working state가 고정 체크리스트와 고정 Strategy Map context보다 높은 정확도를 보였다."
      captionEn: "S&OP ablation results. Within each executor, Full SM working state outperformed ordered checklists and static Strategy Map context."
      columns:
        - key: "executor"
          labelKo: "Executor"
          labelEn: "Executor"
      ...
      rows:
        ...
        - executor: "ReAct"
          ...
          time: "25.3"
        - executor: "ReAct"
          condition: "Full SM working state"
          accuracy: "67.1%"
          delta: "+20.7%p"
          tokens: "275.0"
          time: "98.4"
  ...
  - key: "reflection"
    ...
    bodyKo:
      - >
        이 프로젝트에서 가장 크게 배운 점은 업무 절차 지식을 긴 프롬프트로 넣는 것만으로는 부족하다는 점이다. 복잡한 도구 사용 업무에서는 Strategy Map처럼 현재 무엇이 완료되었고, 무엇이 다음으로 가능하며, 무엇이 아직 막혀 있는지를 실행 중에 관리하는 방식이 더 중요했다.
      - >
      ...
      - >
        또한 단순히 절차를 보여주는 것과 Strategy Map으로 실행 중 상태를 관리하는 것은 달랐다. 순서가 있는 체크리스트나 고정 지도는 업무 흐름을 알려주지만, 현재 세션에서 어떤 선행 조건이 실제로 충족되었는지는 관리하지 못한다. Working Map이 더 좋은 결과를 낸 이유는 이 차이에 있었다.
      - >
    ...
    bodyEn:
      - >
        The main lesson is that workflow knowledge is not enough as long prompt text. In complex tool-use tasks, Strategy Map was useful because it managed what had been completed, what was currently admissible, and what remained blocked during execution.
      - >
      ...
      - >
        It also mattered that static context was not the same as Strategy Map guidance. Ordered checklists and static maps can describe the workflow, but they do not track which prerequisites have actually been satisfied in the current session. The full Working Map was stronger because it maintained this state during execution.
      - >
